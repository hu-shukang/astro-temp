...
test('renders label and calls onClick', async () => {
  ...
  await expect.element(screen.getByRole('button', { name: /save/i })).toBeInTheDocument()
...
test('shows validation error for empty email', async () => {
  ...
  await expect.element(screen.getByText(/email is required/i)).toBeInTheDocument()
})
```
...
test('modal is accessible', async () => {
  const screen = render(<Modal isOpen title="Settings"><p>Content</p></Modal>)
